...
class ConicDesigner:
    ...
    def setup_plot(self):
        ...
        # Рисуем направляющие для касательных
        self.ax.axhline(y=10, color='red', linestyle='--', alpha=0.3, linewidth=1)
        self.ax.axhline(y=-10, color='red', linestyle='--', alpha=0.3, linewidth=1)
        self.ax.axvline(x=-10, color='red', linestyle='--', alpha=0.3, linewidth=1)
        
        # Рисуем фиксированные точки
    ...
    def draw_fixed_points(self):
        ...
        self.ax.plot([self.P0[0]-1, self.P0[0]+1], [self.P0[1], self.P0[1]], 
                    'r-', linewidth=2, alpha=0.7)
        self.ax.text(self.P0[0] + 0.3, self.P0[1], 'P0', fontsize=12, fontweight='bold')
        
        # P4 с горизонтальной касательной
        self.ax.plot(self.P4[0], self.P4[1], 'gs', markersize=12, markeredgewidth=2)
        self.ax.plot([self.P4[0]-1, self.P4[0]+1], [self.P4[1], self.P4[1]], 
                    'r-', linewidth=2, alpha=0.7)
        self.ax.text(self.P4[0] + 0.3, self.P4[1], 'P4', fontsize=12, fontweight='bold')
        ...
        self.ax.plot([self.P2[0], self.P2[0]], [self.P2[1]-1, self.P2[1]+1], 
                    'r-', linewidth=2, alpha=0.7)
        self.ax.text(self.P2[0] - 1.5, self.P2[1], f'P2', fontsize=12, fontweight='bold', color='blue')
        
        # Точки пользователя
        if self.P1:
            self.ax.plot(self.P1[0], self.P1[1], 'ro', markersize=10)
            self.ax.text(self.P1[0] + 0.3, self.P1[1] + 0.3, 'P1', color='red', fontsize=11)
        
        if self.P3:
            self.ax.plot(self.P3[0], self.P3[1], 'mo', markersize=10)
            self.ax.text(self.P3[0] + 0.3, self.P3[1] + 0.3, 'P3', color='magenta', fontsize=11)
    ...
    def fit_conic_liming(self, p_tangent1, p_tangent2, point_on_conic, 
        ...
        p_tangent1 - точка на первой касательной (горизонтальная)
        p_tangent2 - точка на второй касательной (вертикальная)
        point_on_conic - точка на конике
        """
        # Отражаем точку относительно первой касательной (горизонтальной)
        mirrored1 = self.reflect_point_across_line(point_on_conic, p_tangent1, True)
        
        # Отражаем полученную точку относительно второй касательной (вертикальной)
        mirrored2 = self.reflect_point_across_line(mirrored1, p_tangent2, False)
        
        # Собираем точки: 2 точки касательных + исходная + дважды отраженная
        all_points = [p_tangent1, p_tangent2, point_on_conic, mirrored2]
        
        # Составляем матрицу для уравнения коники
        M = []
    ...
    def calculate_and_draw(self):
        """Расчет и отрисовка обоих конических сечений"""
        ...
        # P0 - горизонтальная касательная, P2 - вертикальная касательная
        ...
        # P2 - вертикальная касательная, P4 - горизонтальная касательная
    ...
    def update_info_panel(self, type1=None, type2=None, coeff1=None, coeff2=None):
        ...
        text += f"P0: ({self.P0[0]}, {self.P0[1]})  [горизонт. касат.]\n"
        text += f"P2: ({self.P2[0]:.1f}, {self.P2[1]:.2f}) [вертик. касат.]\n"
        text += f"P4: ({self.P4[0]}, {self.P4[1]})  [горизонт. касат.]\n\n"
